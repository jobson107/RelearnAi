...
const SESSIONS_KEY = 'relearn_sessions';
const SCHEDULE_KEY = 'relearn_schedule';
const GOALS_KEY = 'relearn_goals';
const PROGRESS_KEY = 'relearn_daily_progress';
const POMODORO_SETTINGS_KEY = 'relearn_pomodoro_settings';
const POMODORO_STATS_KEY = 'relearn_pomodoro_stats';

// --- Migration Utility ---
// Automatically migrates old 'synapse_' keys to 'relearn_' keys to preserve user data.
const migrateLegacyData = () => {
  if (typeof window === 'undefined') return;
  
  const migrations = [
    { old: 'synapse_sessions', new: SESSIONS_KEY },
    { old: 'synapse_schedule', new: SCHEDULE_KEY },
    { old: 'synapse_goals', new: GOALS_KEY },
    { old: 'synapse_daily_progress', new: PROGRESS_KEY },
    { old: 'synapse_pomodoro_settings', new: POMODORO_SETTINGS_KEY },
    { old: 'synapse_pomodoro_stats', new: POMODORO_STATS_KEY },
    { old: 'synapse_theme', new: 'relearn_theme' },
    { old: 'synapse_draft_state', new: 'relearn_draft_state' }
  ];

  migrations.forEach(m => {
    const oldData = localStorage.getItem(m.old);
    const newData = localStorage.getItem(m.new);
    if (oldData && !newData) {
      console.log(`[ReLearn] Migrating legacy data: ${m.old} -> ${m.new}`);
