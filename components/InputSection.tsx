...
import { FileText, Upload, Sparkles, X, Loader2, Globe, ScrollText, Sun, Moon, History, ImageIcon, EyeOff, Mic, MicOff } from 'lucide-react';
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 relative z-10">
        ...
        <div className="flex flex-col sm:flex-row gap-3 flex-wrap">
            ...
            <div className="bg-slate-100/80 dark:bg-slate-900/50 p-1 rounded-xl flex items-center border border-slate-200 dark:border-slate-700">
                <button
                type="button"
                onClick={() => setDepth(AnalysisDepth.NOTES_ONLY)}
                className={`flex items-center space-x-2 px-3 py-2 rounded-lg text-sm font-bold transition-all ${depth === AnalysisDepth.NOTES_ONLY ? 'bg-white dark:bg-slate-700 text-indigo-600 dark:text-indigo-400 shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300'}`}
                >
                ...
                <button
                type="button"
                onClick={() => setDepth(AnalysisDepth.DEEP_DIVE)}
                className={`flex items-center space-x-2 px-3 py-2 rounded-lg text-sm font-bold transition-all ${depth === AnalysisDepth.DEEP_DIVE ? 'bg-white dark:bg-slate-700 text-orange-500 dark:text-orange-400 shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300'}`}
                >
