...
export const startVoiceChat = (onVolume: (level: number) => void, onStatusChange: (status: 'connecting' | 'connected' | 'error' | 'disconnected') => void): VoiceChatControl => {
    let nextStartTime = 0;
...
// Simple wrapper to handle responses. In real app, consider retry logic.
...
}, "Offline Summary: Unable to connect to AI.");
...
export const generateStudyAdvice = (text: string) => safeGen('Advice', async () => {
    ...
    return JSON.parse(response.text || '{}') as StudyAdvice;
}, { strategies: ["Review notes manually"], microAdvice: "Focus on key terms." });
...
export const generateStudyRoadmap = (text: string) => safeGen('Roadmap', async () => {
    ...
    return JSON.parse(response.text || '{"items": []}') as RoadmapData;
}, { examType: "General", strategy: "Offline", items: [] });
